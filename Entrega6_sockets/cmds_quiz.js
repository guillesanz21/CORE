...
exports.list = async (socket, rl) => {
  ...
  quizzes.forEach(q =>
    socket.write(`  "${q.question}" (by ${q.author.name}, id=${q.id})`)
  );
};
...
exports.create = async (socket, rl) => {
  ...
  socket.write(`   User ${name} creates quiz: ${question} -> ${answer}`);
};

// Test (play) quiz identified by <id>
exports.test = async (socket, rl) => {
  ...
  if (answered.toLowerCase().trim() === quiz.answer.toLowerCase().trim()) {
    socket.write(`  The answer "${answered}" is right!`);
  } else {
    socket.write(`  The answer "${answered}" is wrong!`);
  }
};
...
exports.update = async (socket, rl) => {
  ...
  socket.write(`  Quiz ${id} updated to: ${question} -> ${answer}`);
};
...
exports.delete = async (socket, rl) => {
  ...
  socket.write(`  ${id} deleted from DB`);
};
